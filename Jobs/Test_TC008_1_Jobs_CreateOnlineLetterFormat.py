...
class Test_LE007_Job:
    ...
    def Sheet():
        ...
        for r in range(1, rows, 1):
            ...
            style = sheet.cell_value(r, 9)
            Font = int(sheet.cell_value(r, 10))
            Upload = sheet.cell_value(r, 11)
            Size = sheet.cell_value(r, 12)

            Tuple = (name, ForwardEmail, UpdateAddress, Notes, Address, City, State, Zip, style, Font, Upload, Size)
            List.append(Tuple)
        return List

    @pytest.mark.parametrize("name, ForwardEmail, UpdateAddress, Notes, Address, City, State, Zip, style, Font, Upload, Size", Sheet())
    def test_LE007_Job(self, test_setup, name, ForwardEmail, UpdateAddress, Notes, Address, City, State, Zip, style, Font, Upload, Size):
        ...
        time.sleep(5)
        self.job.SelectStyele(style)
        time.sleep(5)
        self.job.SelectFont(Size)
        time.sleep(5)
