...
class Test_TC032_1_Contacts_CloneContact:
    ...
    def Sheet():
        ...
        sheet = wb.sheet_by_name("Contacts_CloneContact")
        List = []
        rows = sheet.nrows
        for r in range(1, rows, 1):
            ...
            Clone_name = sheet.cell_value(r, 25)
            ...
            Tuple = (conName, FName, MName, LName, Prefix, Suffix, JobTitle, Company, Notes, Tags, Category_type,
                     ...
                     date_purpose, date, url_purpose, url, Clone_name)
            List.append(Tuple)
        return List
    ...
    @pytest.mark.parametrize("conName, FName, MName, LName, Prefix, Suffix, JobTitle, Company, Notes, Tags, "
                             ...
                             " email_purpose, email, date_purpose, date, url_purpose, url, Clone_name", Sheet())
    def test_TC032_1_CloneContact(self, test_setup, conName, FName, MName, LName, Prefix, Suffix, JobTitle, Company, Notes,
                           Tags, Category_type, AddLine1, AddLine2, city, state, zipCode, Country, phone_purpose,
                           phoneNo, email_purpose, email, date_purpose, date, url_purpose, url, Clone_name):
        self.driver = test_setup
        self.logger.info("********** Test_TC032_1_Contacts_CloneTheContact TestCase Started *********")
        time.sleep(3)
        ...
        time.sleep(2)
        self.con.clkOnCloneBtn()
        time.sleep(2)
        self.con.EditBusinessContact_Details(JobTitle, Company, FName, MName, LName, Prefix, Suffix, Notes)
        ...
        time.sleep(3)
        self.con.clkOnCloneSaveBtn()
        time.sleep(12)
        self.con.clkOnserachField(Clone_name)
        time.sleep(5)
        ...
        try:
            if "Contacts" in self.msg:
                assert True
                allure.attach(self.driver.get_screenshot_as_png(), name="Successfully Cloned the Contact in Contacts",
                              attachment_type=AttachmentType.PNG)
                self.logger.info("******* Successfully cloned the Contact in Contacts *******")
                self.logger.info("Test_TC032_1_Contacts_CloneTheContact is Passed")
                time.sleep(2)
                self.lo.Logout()
            else:
                allure.attach(self.driver.get_screenshot_as_png(), name="Failed while clone the Contact in Contacts",
                              attachment_type=AttachmentType.PNG)
                self.logger.info("******* Failed while clone the Contact in Contacts *******")
                self.logger.info("Test_TC032_1_Contacts_CloneTheContact is Failed")
                time.sleep(2)
                assert False
