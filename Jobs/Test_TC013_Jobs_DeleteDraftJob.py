...
class Test_TC013_Jobs_DeleteDraftJob:
    ...
    def Sheet():
        ...
        sheet = wb.sheet_by_name("Jobs_DeleteDraftJob")
        List = []
        rows = sheet.nrows
    ...
    def test_TC013_DeleteDraftJob(self, test_setup, Status, Date, GroupBatches, Action, Name):
        self.driver = test_setup
        self.logger.info("********** Test_TC013_Jobs_DeleteDraftJob TestCase Started *********")
        time.sleep(3)
        ...
        time.sleep(3)
        self.job.ClkOnDraftJob_DeleteBtn(Name)
        time.sleep(3)
        ...
        # try:
        #     toaster = WebDriverWait(self.driver, 5).until(
        #         EC.presence_of_element_located((By.XPATH, '//div[@class="growl-message"]'))
        #     )
        #     # print(toaster.text)
        #     # self.driver.find_element(By.XPATH, '//div[@ng-bind-html="message.text"]')
        #     time.sleep(2)
        #     self.msg = toaster.text
        #     print(self.msg)
            # self.msg = self.driver.find_element(By.TAG_NAME, "body").text
            # print(self.msg)

            if "Yes! We successfully deleted Mailing Draft.." in toaster_text:
                assert True
                allure.attach(self.driver.get_screenshot_as_png(), name="Successfully Deleted the Drafted Job",
                              attachment_type=AttachmentType.PNG)
                self.logger.info("******* Successfully Deleted the Drafted Job *******")
                self.logger.info("Test_TC013_Jobs_DeleteDraftJob is Passed")
                time.sleep(5)

            else:
                allure.attach(self.driver.get_screenshot_as_png(), name="Failed while Delete the Drafted Job",
                              attachment_type=AttachmentType.PNG)
                self.logger.info("******* Failed while Delete the Drafted Job *******")
                self.logger.info("Test_TC013_Jobs_DeleteDraftJob is Failed")
                time.sleep(2)
                assert False
            self.lo.Logout()
            time.sleep(2)
